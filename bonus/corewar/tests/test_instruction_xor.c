...
Test(instruction_xor, error_1)
{
    cr_assert(instruction_xor(NULL, NULL) == 84);
}

Test(instruction_xor, error_2)
{
    vm_t vm = {0};

    cr_assert(instruction_xor(&vm, NULL) == 84);
}

Test(instruction_xor, error_3)
{
    champion_t champion = {0};

    cr_assert(instruction_xor(NULL, &champion) == 84);
}

Test(instruction_xor, success_1)
{
    vm_t vm = {0};
    champion_t champion = {0};
    ...
    cr_assert(!instruction_xor(&vm, &champion));
}

Test(instruction_xor, success_1_1_eq)
{
    vm_t vm = {0};
    champion_t champion = {0};
    ...
    my_memcpy("\x07\xa4\x00\x00\x00\x00\x00\x00\x00\x01\x01", vm.memory, 11);
    if (instruction_xor(&vm, &champion) == 84)
        cr_assert(0);
    cr_assert(*(int *)(champion.registers + (REG_SIZE * 1)) == 1);
}

Test(instruction_xor, success_1_2_eq)
{
    vm_t vm = {0};
    champion_t champion = {0};
...
Test(instruction_xor, success_2)
{
    vm_t vm = {0};
    champion_t champion = {0};
...
Test(instruction_xor, success_2_1_eq)
{
    vm_t vm = {0};
    champion_t champion = {0};
...
Test(instruction_xor, success_2_2_eq)
{
    vm_t vm = {0};
    champion_t champion = {0};
    champion.registers = malloc(REG_SIZE * 2);
